...
map<int, Node *> nodeMap[N + 1];  // nodeMap[x] := x번 세로줄의 정점들의 y
                                  // 좌표를 들고 있는 TreeMap
Node *newNode;

// 2가지가 필요한데, set 100개, node 200000*2개
// 기본적으로 head tail 쌍 100개 만들고 add할때마다 노드 2개씩 생성
// 이중연결리스트로 구현
// treeset은 노드를 어디에 삽입하는지 찾는거고, 이중연결리스트는 조회할때 사용

void init() {
  for (int i = 1; i <= N; i++) {
    nodeMap[i].clear();
    nodeMap[i][0] = node + i;  // i 번 세로줄에 i 번 참가자를 놓는다.
    nodeMap[i][1000000000] =
        node + FIRST_END_NODE + i - 1;  // i 번 세로줄의 도착점을 추가한다.
...
void add(int mX,
         int mY)  // (mX, mY)와 (mX + 1, mY)을 잇는 가로줄 추가하기, O(Log N)
{
  Node *nowLeft = newNode++;
  Node *nowRight = newNode++;

  Node *prevLeft = (--nodeMap[mX].upper_bound(mY))
                       ->second;  // mX 번 세로 줄에서 mY 직전 정점 찾기
  Node *prevRight = (--nodeMap[mX + 1].upper_bound(mY))
                        ->second;  // (mX + 1) 번 세로 줄에서 mY 직전 정점 찾기

  Node *nextLeft = prevLeft->next;  // mX 번 세로 줄에서 mY 직후 정점 찾기
  Node *nextRight =
      prevRight->next;  // (mX + 1) 번 세로 줄에서 mY 직후 정점 찾기
...
void remove(int mX,
            int mY)  // (mX, mY)와 (mX + 1, mY)을 잇는 가로줄 제거하기, O(Log N)
{
  Node *nowLeft = nodeMap[mX][mY];  // mX번 세로 줄에서 mY 위치 정점 찾기
  Node *nowRight =
      nodeMap[mX + 1][mY];  // (mX + 1)번 세로 줄에서 mY 위치 정점 찾기
...
int numberOfCross(
    int mID)  // mID번 참가자가 지나게 되는 가로줄의 개수 구하기, O(5000)
{
  int ret = -1;
  Node *now = node + mID;  // mID 번 세로줄에서 출발하기
  while (now <
         node + FIRST_END_NODE)  // 마지막 줄에 도착할 때까지 다음으로 이동하기
  {
    ret++;
    now = now->next;
  }
...
{
  Node *now = (--nodeMap[mX].upper_bound(mY))
                  ->second;  // mX 번 세로 줄에서 mY 직전 정점 찾기
  while (now >
         node + LAST_START_NODE)  // 최상단에 도착할 때까지 이전으로 돌아가기
    now = now->prev;
  return now - node;
}